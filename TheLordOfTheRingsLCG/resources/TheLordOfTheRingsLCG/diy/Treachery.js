...
function createTemplates( diy ){
	diy.frontTemplateKey = 'Treachery-front';
...
/* TEMPLATE */
	var combo = new Array('Standard','Nightmare','Burden','Gold','CustomDifficulty');
...
/* TEXT */
	...
	RulesTab.place(
		...
		separator(),'newline,growx',
		@LRL-Shadow,'newline,center',Shadow_text,'newline,growx',
		separator(),'newline,growx',
...
/* TEMPLATE */
	...
	case 'Gold':
		...
		sheet.paintImage(g,DifficultyDeco_tinter.getTintedImage(),'Treachery-difficultyDeco-region');
		break;
	case 'CustomDifficulty':
		...
		sheet.paintImage(g,DifficultyDeco_tinter.getTintedImage(),'Treachery-difficultyDeco-region');
		break;
	case 'Nightmare': case 'Burden':
		sheet.paintImage(g,'Treachery-'+$Template+'-front-template',0,0);
		break;
	default:
		sheet.paintTemplateImage(g);
	}
...
/* TEXTS */
	...
	const BODY_SHAPE_BURDEN = new PageShape.CupShape( 64, 24, 338, 0, 0 );
	const BODY_SHAPE = new PageShape.CupShape( 0, 0, 0, 0, 0 );
	if( String( $Template ) == 'Burden' ){
		Body_box.setPageShape(BODY_SHAPE_BURDEN);
	}else{
